...
try
{
    ...
    # Change project TargetFrameworkVersion from 4.0 or 4.5.1 to 4.5.2
    $configContent = Get-Content $project.FullName -Raw
    ...
    $configContent = $configContent.Replace('<TargetFrameworkVersion>v4.5.1</TargetFrameworkVersion>', '<TargetFrameworkVersion>v4.5.2</TargetFrameworkVersion>');
    Set-Content -Path $project.FullName -Value $configContent

    # Iterate through each web.config (root, scrripts, content, views) and make necessary changes
    $projectXml.DocumentElement.SelectNodes("//mvc3:Content[contains(translate(@Include,'WEBCONFIG','webconfig'),'web.config')]", $nameSpaceManager) | ForEach-Object { $config = $projectDirectory + "\" + $_.Include; $configContent = [System.IO.File]::ReadAllText($config); $configContent = $configContent.Replace('System.Web.Mvc, Version=3.0.0.0', 'System.Web.Mvc, Version=5.2.3.0'); $configContent = $configContent.Replace('System.Web.Mvc, Version=3.0.0.1', 'System.Web.Mvc, Version=5.2.3.0'); $configContent = $configContent.Replace('System.Web.Mvc, Version=4.0.0.0', 'System.Web.Mvc, Version=5.2.3.0'); $configContent = $configContent.Replace('System.Web.Mvc, Version=4.0.0.1', 'System.Web.Mvc, Version=5.2.3.0'); [System.IO.File]::WriteAllText($config, $configContent); }
    $projectXml.DocumentElement.SelectNodes("//mvc3:Content[contains(translate(@Include,'WEBCONFIG','webconfig'),'web.config')]", $nameSpaceManager) | ForEach-Object { $config = $projectDirectory + "\" + $_.Include; $configContent = [System.IO.File]::ReadAllText($config); $configContent = $configContent.Replace('System.Web.WebPages, Version=1.0.0.0', 'System.Web.WebPages, Version=3.0.0.0'); $configContent = $configContent.Replace('System.Web.Helpers, Version=1.0.0.0', 'System.Web.Helpers, Version=3.0.0.0'); $configContent = $configContent.Replace('System.Web.WebPages.Razor, Version=1.0.0.0', 'System.Web.WebPages.Razor, Version=3.0.0.0'); $configContent = $configContent.Replace('System.Web.WebPages, Version=2.0.0.0', 'System.Web.WebPages, Version=3.0.0.0'); $configContent = $configContent.Replace('System.Web.Helpers, Version=2.0.0.0', 'System.Web.Helpers, Version=3.0.0.0'); $configContent = $configContent.Replace('System.Web.WebPages.Razor, Version=2.0.0.0', 'System.Web.WebPages.Razor, Version=3.0.0.0'); [System.IO.File]::WriteAllText($config, $configContent); }
    ...
    $webPageVersion = $rootConfig.DocumentElement.SelectSingleNode("/configuration/appSettings/add[@key='webpages:Version']")
    if($webPageVersion)
    {
        $webPageVersion.Value = "3.0.0.0"
    }

    # Changing root web.config runtime targetFramework version
    #$rootConfig.DocumentElement."system.web".httpRuntime.targetFramework = "4.5.2"
    $httpRuntime = $rootConfig.DocumentElement.SelectSingleNode("/configuration/system.web/httpRuntime")
    if($httpRuntime)
    {
        if(-Not ($httpRuntime.targetFramework))
        {
            $httpRuntime.SetAttribute("targetFramework", "4.5.2")
        }
        else
        {
    ...
    # Change project type MVC 3/4 to MVC 5
    ...
    $configContent = $configContent.Replace('E3E379DF-F4C6-4180-9B81-6769533ABE47', '349C5851-65DF-11DA-9384-00065B846F21');
